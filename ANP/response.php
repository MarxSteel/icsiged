...
$connecta = mysqli_connect("localhost:8889", "root", "root", "siged_novo") or die("Connection failed: " . mysqli_connect_error());
...
if (mysqli_connect_errno()) {
	...
	if( !empty($params['searchPhrase']) ) {   
		$where .=" WHERE ";
		$where .=" ( pro_nome LIKE '".$params['searchPhrase']."%' ";    
		$where .=" OR pro_avenida LIKE '".$params['searchPhrase']."%' )";
		$where .=" OR pro_DtCadastro LIKE '".$params['searchPhrase']."%' )";
	}
	
	// getting total number records without any search
	$sql = "SELECT * FROM `icbr_projeto`";
	$sqlTot .= $sql;
	$sqlRec .= $sql;
